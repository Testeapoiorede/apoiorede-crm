    ...
    return (
      <main style={loginPageStyle}>
        <section style={loginCardStyle}>
          ...
          <p style={{ color: "#64748b", marginBottom: 24 }}>CRM operacional odontológico</p>
    ...
    <main style={appStyle}>
      ...
      <section style={mainStyle}>
        ...
        <div style={metricsStyle}>
          <div style={metricCard}><span>Total</span><strong>{requests.length}</strong></div>
          <div style={metricCard}><span>Pendentes</span><strong style={{ color: "#d97706" }}>{requests.filter((r) => r.status === "PENDENTE").length}</strong></div>
          <div style={metricCard}><span>Aprovadas</span><strong style={{ color: "#16a34a" }}>{requests.filter((r) => r.status === "APROVADO").length}</strong></div>
          <div style={metricCard}><span>Clínicas</span><strong style={{ color: "#2563eb" }}>{clinics.length}</strong></div>
        </div>
      ...
      {showRequestModal && (
        <div style={modalOverlay}>
          <div style={modalCard}>
            ...
            <textarea placeholder="Observação" value={observation} onChange={(e) => setObservation(e.target.value)} style={{ ...inputStyle, minHeight: 100 }} />
